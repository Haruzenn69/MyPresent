<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-xl text-gray-800">Tambah Siswa</h2>
    </x-slot>
            ...
            <div class="card p-6">
                <form method="POST" action="{{ route('admin.students.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nis" value="{{ old('nis') }}" class="input w-full @error('nis') border-red-500 @enderror" required>
                        @error('nis')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>

                    <div class="mb-4">
                        <label class="input-label">Nama</label>
                        <input type="text" name="nama" value="{{ old('nama') }}" class="input w-full @error('nama') border-red-500 @enderror" required>
                        @error('nama')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>

                    <div class="mb-4">
                        <label class="input-label">Email</label>
                        <input type="email" name="email" value="{{ old('email') }}" class="input w-full @error('email') border-red-500 @enderror" required>
                    ...
                    <div class="mb-4 grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="password" name="password" class="input w-full @error('password') border-red-500 @enderror" required>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="alamat" rows="3" class="input w-full @error('alamat') border-red-500 @enderror">{{ old('alamat') }}</textarea>
